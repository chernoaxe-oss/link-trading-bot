...
ESTADO_FILE = "estado_scalping_q.json"

# Capital $300 → Trading Power $6,000 → Posición 30% = $1,800
TRADING_POWER = 6000
...
def cargar_estado():
    ...
    try:
        ...
        if en_operacion:
            enviar_mensaje(f"🔄 Bot Scalping Q reiniciado con operación abierta\n{direccion} desde ${precio_entrada:.4f}")
        elif esperando_confirmacion:
            esperando_confirmacion = False
            ciclos_esperando = 0
            guardar_estado()
            enviar_mensaje("🔄 Bot Scalping Q reiniciado — señal pendiente cancelada")
        else:
            enviar_mensaje("⚡ Bot Scalping Quantfury iniciado\n1H+15m+5m → 1m | Posición $1,800")
    except Exception as e:
        enviar_mensaje(f"⚠️ Error cargando estado scalping Q: {str(e)}")
...
def registrar_operacion(resultado, p_entrada, p_salida, porcentaje, ganancia_dolares):
    ...
    data = {
        ...
        "properties": {
            "Nombre": {"title": [{"text": {"content": f"LINK Scalping Q - {datetime.now().strftime('%d/%m/%Y %H:%M')}"}}]},
            "PLATAFORMA": {"select": {"name": "Quantfury"}},
...
def verificar_senal():
    ...
    try:
        ...
        # Descargar datos — contexto en 1H, 15m, 5m — entrada en 1m
        ...
        if en_operacion:
            if direccion == 'LONG':
                if rsi_anterior >= 70 and precio_anterior >= bb_sup_ant:
                    ...
                    guardar_estado()
                    enviar_mensaje(
                        f"{'✅' if resultado=='TP' else '❌'} SALIDA LONG — {resultado} ⚡ SCALPING Q\n"
                ...
                if rsi_anterior <= 30 and precio_anterior <= bb_inf_ant:
                    ...
                    guardar_estado()
                    enviar_mensaje(
                        f"{'✅' if resultado=='TP' else '❌'} SALIDA SHORT — {resultado} ⚡ SCALPING Q\n"
                        f"Entrada: ${precio_entrada:.4f} | Salida: ${precio_actual:.4f}\n"
                        f"{porcentaje:.2f}% | ${ganancia:.2f}\n"
                        f"⚡ Cerrar posición en Quantfury AHORA"
                    )
        ...
        if esperando_confirmacion:
            ...
            if mensaje == "si":
                ...
                enviar_mensaje(
                    f"🟢 ENTRADA CONFIRMADA — {dir_txt} SCALPING Q\n"
                    f"Precio: ${precio_entrada:.4f}\n"
                    f"Posición: $1,800 (30% de $6,000 TP)\n"
                    f"⚡ Entrar en Quantfury AHORA\n"
                ...
                if ciclos_esperando >= CICLOS_MAX_ESPERA:
                    ...
                    alerta_temprana_ctx = None
                    guardar_estado()
                    enviar_mensaje("⏱️ Señal Scalping Q cancelada por tiempo")
            return
        ...
        if banda_tocada and alerta_temprana_ctx != dir_ctx:
            ...
            enviar_mensaje(
                f"⚠️ ALERTA SCALPING Q — {dir_ctx} {'📈' if dir_ctx=='LONG' else '📉'}\n"
        ...
        enviar_mensaje(f"⚠️ Error bot scalping Q: {str(e)}")
...
while True:
    now = datetime.now()
    print(f"{now.strftime('%H:%M:%S')} - Scalping Q verificando...")
    verificar_senal()
    time.sleep(30)  # Cada 30 segundos por ser 1m
